...
$__System.register('6', ['5'], function (_export) {
  ...
  var blankTemplate, subTitleTemplate, inputTemplate, datepickerTemplate, textareaTemplate, richTextTemplate, checkboxTemplate, radioTemplate, basicSelectTemplate, basicMultiSelectTemplate, groupedSelectTemplate, groupedMultiSelectTemplate, uploadTemplate, uploadTemplateListFiles, validationTemplate, validationTemplateMultiple, descriptionTemplate, visibilityTemplate;
  ...
  function formlyConfig(formlyConfigProvider) {
    ...
    formlyConfigProvider.setType({
      ...
      wrapper: ['validationTemplateMultiple', 'descriptionTemplate', 'uploadTemplateListFÏiles', 'bootstrapLabel', 'bootstrapHasError'],
      defaultOptions: {
        noFormControl: true
      },
      controller: ['$scope', function ($scope) {
...
$__System.registerDynamic("7", [], true, function ($__require, exports, module) {
    ...
    module.exports = {
        ...
        "EVENT_DEPENDENCY": "Dependency is specified at events section",
        ...
        "REFRESH_DB_SELECT": "Refresh list from DB",
	...
	return {
		...
		execute: function () {
			...
			edaStepWayEasyFormGenController = (function () {
				function edaStepWayEasyFormGenController(easyFormGenVersion, $filter, toaster, $timeout, $uibModal, $log, $formlyProxy, $modalProxy, $scope, $translate, easyFormSteWayConfig) {
				...
				_createClass(edaStepWayEasyFormGenController, [{
					...
					value: function init() {
						...
						this.populateSourceFields = function () {
							...
							angular.forEach(this.fieldsSpecial, function (field) {
								field.count++;
								field.index++;
						...
						this.optionsSourceDbFormatConfig = {
							...
							onItemAdd: function onItemAdd(value) {
								if (!edaController.fieldsInit) {
									var matches = value.match(/\[(OPTIONAL_START|OPTIONAL_END|SPACE)_([0-9]*)\]/);
									if (matches && matches.length == 3 && matches[1] && matches[1].length > 0 && matches[2] && matches[2].length > 0) {
										var fieldSpecial = edaController.fieldsSpecial[matches[1]];
										fieldSpecial.count++;
										fieldSpecial.index++;
							...
							onItemRemove: function onItemRemove(value) {
								if (!edaController.fieldsInit) {
									...
									if (matches && matches.length == 3 && matches[1] && matches[1].length > 0 && matches[2] && matches[2].length > 0) {
										...
										var index = -1;
										angular.forEach(edaController.fields, function (field, key) {
					...
					key: 'prepareAllOptions',
					value: function prepareAllOptions() {
						var options = {};

						angular.forEach(this.configuration.lines, function (line, lineKey) {
							angular.forEach(line.columns, function (column, columnKey) {
								if (column.control.type == 'radio' || column.control.type == 'basicSelect' || column.control.type == 'groupedSelect') {
									angular.forEach(column.control.templateOptions.options, function (option, optionKey) {
						...
						angular.extend(editControlModal, {
							...
							resolve: {
								...
								allOptions: function allOptions() {
									return _this3.prepareAllOptions();
								}
							}
						});
			...
			toInject = ['easyFormGenVersion', '$filter', 'toaster', '$timeout', '$uibModal', '$log', '$formlyProxy', '$modalProxy', '$scope', '$translate', 'easyFormSteWayConfig'];
	...
	return {
		...
		execute: function () {
			...
			editControlModalController = (function () {
				function editControlModalController($uibModalInstance, nyaSelect, toaster, selectOptionManage, $modalProxy, titleColumns, columns, activeLine, activeColumn, activeLineColumnsCount, allOptions, $scope, $translate) {
					var _this2 = this;
				...
				_createClass(editControlModalController, [{
					...
					value: function init() {
						...
						this.optionsSourceDbFormatConfig = {
							...
							onItemAdd: function onItemAdd(value) {
								if (!edaController.fieldsInit) {
									var matches = value.match(/\[(OPTIONAL_START|OPTIONAL_END|SPACE|VALUE)_([0-9]*)\]/);
									if (matches && matches.length == 3 && matches[1] && matches[1].length > 0 && matches[2] && matches[2].length > 0) {
										var fieldSpecial = edaController.fieldsSpecial[matches[1]];
										fieldSpecial.count++;
										fieldSpecial.index++;
							...
							onItemRemove: function onItemRemove(value) {
								if (!edaController.fieldsInit) {
									...
									if (matches && matches.length == 3 && matches[1] && matches[1].length > 0 && matches[2] && matches[2].length > 0) {
										...
										var index = -1;
										angular.forEach(edaController.optionsSourceDbFields, function (field, key) {
											if (field.value == value) {
												index = key;
											}
										});
					...
					value: function addNewEvent() {
						var result = this.selectOptionManage.addNewEvent(this.nyaSelect.temporyConfig.eventsList, this.newEvent);
						if (result.resultFlag === false) {
							this.toaster.pop({
								type: 'warning',
								timeout: 2000,
					...
					value: function removeEvent(index) {
						var result = this.selectOptionManage.removeEvent(this.nyaSelect.temporyConfig.eventsList, index);
						if (result.resultFlag === false) {
							this.toaster.pop({
								type: 'warning',
								timeout: 2000,
					...
					key: 'resetEventTypes',
					value: function resetEventTypes(resetEvents) {
						var _this3 = this;
						...
						if (typeof resetEvents == 'object' && resetEvents.length > 0) {
							(function () {
								var offset = 0;
								var events = angular.copy(_this3.nyaSelect.temporyConfig.eventsList);
								...
								angular.forEach(events, function (event, index) {
									if (event.type && event.type.id && resetEvents.indexOf(event.type.id) >= 0) {
										_this3.removeEvent(index - offset);
										offset++;
									}
								});
							})();
			...
			toInject = ['$uibModalInstance', 'nyaSelect', 'toaster', 'selectOptionManage', '$modalProxy', 'titleColumns', 'columns', 'activeLine', 'activeColumn', 'activeLineColumnsCount', 'allOptions', '$scope', '$translate'];
...
$__System.register('1e', [], function (_export) {
	...
	return {
		...
		execute: function () {
			...
			_export('extractTemplateOptionEventsList', extractTemplateOptionEventsList);
	...
	return {
		...
		execute: function () {
			...
			selectOptionManage = (function () {
				function selectOptionManage($translate) {
				...
				_createClass(selectOptionManage, [{
					...
					key: 'addNewEvent',
					value: function addNewEvent(selectObj, newEvent) {
						var fullResponse = {
							resultFlag: false,
							details: ''
						};

						var checkResult = this.validateEvent(selectObj, newEvent);
					...
					key: 'removeEvent',
					value: function removeEvent(selectObj, AtIndex) {
						var fullResponse = {
							resultFlag: false,
							details: ''
						};
					...
					key: 'validateEvent',
					value: function validateEvent(selectObj, newEvent) {
						var _this = this;
						...
						angular.forEach(selectObj, function (event) {
							if (angular.equals(event, newEvent)) {
								valid = false;
								details = _this.$translate.instant('ALREADY_EXISTS');
							}
						});
			...
			selectOptionManage.$inject = ['$translate'];
